...
class MoodRecordController extends Controller
{
    ...
    /**
     * Menampilkan daftar mood record untuk user yang sedang login.
     *
        ...
        if (strpos($acceptHeader, 'text/vnd.turbo-stream') !== false) {
            ...
            if ($records->count() > 0) {
                foreach ($records as $record) {
                    $recordsContent .= view('components.mood-record-item', [
                        'record' => $record,
                        'user' => Auth::user()
                    ])->render();
                }
            ...
            $streamContent = TurboStreamHelper::replace('record_container_list', $recordsContent);
            $streamContent .= TurboStreamHelper::replace('pagination-container', $pagination);
    ...
    /**
     * Menampilkan halaman pagination mood record.
     *
     * @param  \Illuminate\Http\Request  $request
     * @return \Illuminate\Http\Response
     */
